...
void connect_to_broker()
{
  while (!client.connected())
  {
    ...
    else
    {
      ...
      Serial.println("Reload 5 seconds");
      delay(5000);
    }
  }
}
...
{
  Serial.println("------------Reciver from topic----------");
  Serial.print("topic: ");
...
void setup()
{
  ...
  // Setup port and address hivemq
  client.setServer(MQTT_SERVER, MQTT_PORT);
/*
  Register a setcallback function in the pubsubclient library that will be called when the library receives a new value changed in the broker for the data the topic is subscribed to.
*/
...
void sendMessageLora(String outgoing, byte MasterNode, byte otherNode)
{                               // start packet
...
void printData(String tempValue, String TDSValue, String turbidityValue, String pHValue)
{
  ...
  {
    Serial.println(("Error read values in Receiver Lora"));
  }
  else
  {
